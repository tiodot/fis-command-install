...
function Remote(address, options) {
    ...
    this.resolve = function () {
        ...
        return Promise
            ...
            .try(function () {
                ...
                var packagePath = path.join(componentsDir, self.name, 'package.json');
                var componentPath = path.join(componentsDir, self.name, 'component.json');
                var filepath = exists(packagePath) ? packagePath : componentPath;

                if (filepath === packagePath) {
                    // 如果是packge方式，设置protocol为npm
                    factory.settings.protocol = 'npm';
                }
                ...
                if (exists(filepath)) {
                    ...
                    return getConfig(filepath)
                        ...
                        .then(function (json) {
                            ...
                            if (semver.satisfies(json.version, version)) {
                                resolved = json.version;
                            }
    ...
    this.resolveVersion = function (versions) {
        if (!this.version) {
            ...
            if (!resolved && !/\d/.test(version) && ~versions.indexOf('master')) {
                resolved = 'master';
            }
            ...
            // console.log('%s@%s', this.address, this.version)
        }
    ...
    this.convert = function (location) {
        // reload local config.
        var config = loadConfig(path.join(location, 'component.json'));
        if (!config) {
            config = loadConfig(path.join(location, 'package.json'));
        }
